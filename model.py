...
class SCNN(nn.Module):
    def __init__(self):
        ...
        self.fc9 = nn.Linear(2400, 128) # 4500-->128
        self.fc10 = nn.Linear(128, 4)  #128-->4   4 labels
    ...
    def forward(self, x): # 0.44 ~ 0.49s
        # ------ time-consuming: 0.43

        x = F.relu(self.bn1_1(self.conv1_1(x)))  # ---0.004s
        x = F.relu(self.bn1_2(self.conv1_2(x)))
        x = F.max_pool2d(x, 2, stride=2, padding=0)
        # print 'raw',x.shape
        x = F.relu(self.bn2_1(self.conv2_1(x))) # --0.0035s
        x = F.relu(self.bn2_2(self.conv2_2(x)))
        x = F.max_pool2d(x, 2, stride=2, padding=0)

        x = F.relu(self.bn3_1(self.conv3_1(x))) # --0.005s
        x = F.relu(self.bn3_2(self.conv3_2(x)))
        x = F.relu(self.bn3_3(self.conv3_3(x)))
        x = F.max_pool2d(x, 2, stride=2, padding=0)

        x = F.relu(self.bn4_1(self.conv4_1(x))) # --0.006s
        x = F.relu(self.bn4_2(self.conv4_2(x)))
        x = F.relu(self.bn4_3(self.conv4_3(x)))


        x = F.relu(self.bn5_1(self.conv5_1(x))) # total 0.25 --0.081s

        x = F.relu(self.bn5_2(self.conv5_2(x))) # --0.081s
        # torch.cuda.synchronize()
        # start = time.time()
        x = F.relu(self.bn5_3(self.conv5_3(x))) # --0.081

        # torch.cuda.synchronize()
        # end = time.time()
        # print '00time', end - start
        x = F.relu(self.bn6(self.conv6(x))) # --0.17s
        ...
        x = self.conv8(x)
        # print x.shape
        x1 = F.interpolate(x, size=[270,480], mode='bilinear', align_corners=False)
        ...
        return x1, x2
        #x1 predictmaps, predicts (1 1 1 1)
